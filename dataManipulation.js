...
const buildVizJson = (data) => {
  let totalVideos = generateTotalVideos(data);
  ...
  const timeWatched = generateTimeWatched(data);

  const channelOverTime = generateChannelOverTime(data);
  const mostWatchedChannelsData = sortAndReduceChannelOverTime(channelOverTime);


  const mostWatchedChannelsTop = generateMostWatchedChannels(channelOverTime);


  const channelOverTimeVideos = generateChannelOverTimeVideos(data);
  const mostWatchedChannelsVideosData = sortAndReduceChannelOverTime(channelOverTimeVideos);
  return {
    watchedDaily: watchedDaily,
    timeWatched: timeWatched,
    totalVideos: totalVideos,
    channelOverTime: channelOverTime,
    mostWatchedChannelsVideosData: mostWatchedChannelsVideosData,
    mostWatchedChannelsTop:mostWatchedChannelsTop,
    mostWatchedChannelsData: mostWatchedChannelsData,
  }
}

const generateTotalVideos = (data) =>{

  let output = data.length.toLocaleString();
  return output;
